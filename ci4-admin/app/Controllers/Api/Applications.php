...
/**
 * 참가신청 관리 (adminAuth 필터 뒤)
 * - GET  /api/apps : 전체 목록(최신순) — 프론트 계약상 배열을 그대로 반환
 * - POST /api/apps : {id, status?, memo?} 부분 수정
 */
class Applications extends ApiController
{
    public function index(): ResponseInterface
    {
        $rows = model(ApplicationModel::class)->orderBy('created_at', 'DESC')->findAll();
